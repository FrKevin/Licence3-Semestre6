...
unsigned char convertToChar(unsigned char c) {
  int i = 0;
  unsigned int res = 0;
  for(i = 0; i <= 7; i++) {
    res += pow(2,i)*(c%2);
...
unsigned int convertToBytes(unsigned char c) {
  int i = 0;
  unsigned int res = 0;
  for(i = 0; i <= 7; i++) {
    res += pow(2,i)*(c%2);
...
unsigned char* convertToDNSNameFormat(unsigned char* host) {
  unsigned char* dns;
  int lock = 0 , i;
  strcat((char*)host,".");
...
int main() {
  ...
  char ** p = NULL;
  char minibuffchar[2]; /* Sert pour la convertion des hexas*/
  char minibuffint; /* Sert pour la convertion des hexas*/
  ...
  for( i=0 ; i<len ; i++){
    ...
    fprintf(stdout,"%c ",(char)convertToChar(buffer[i]));
    if (!((i+1)%16))
      fprintf(stdout,"\n");
  }

  fprintf(stdout, "\e[39m"); // Affichage blanc

  fprintf(stdout,"\n\nAdresse IP du site : \n");
  minibuffchar[1] = 'l';
  for( i=61 ; i<=64 ; i++){
      fprintf(stdout, "\e[93m%i", convertToBytes(buffer[i]));
      if(i<64) fprintf(stdout, "\e[39m.");
  }
